...
def process(src_file, dest_dir, start_dir, template_file):
    ...
    with open(src_file, 'r') as source:
        for line in source:
            if '<!-- #include' in line:
                include_file = line.split('<!-- #include')[1].split('-->')[0].strip()
        ...
        with open(dest_file, 'w') as destination:
            for template_line in template:
                if '##content##' in template_line:
                    ...
                    destination.write(template_line.replace('##title##', title))
                else:
...
for root, dirs, files in os.walk(start_dir):
    for file in files:
        ...
        process(src_file, dest_dir, start_dir, template_file)
